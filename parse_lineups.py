...
def _combine_headers(rows: Sequence[Sequence[str]]) -> Tuple[List[str], List[List[str]]]:
    ...
    main, sub, *rest = rows
    combined: List[str] = []
    for primary, secondary in zip_longest(main, sub, fillvalue=""):
...
def _normalize_key(label: str) -> str:
    """Simplify a label for fuzzy matching (alphanumeric, lowercase)."""

    return re.sub(r"[^a-z0-9]+", "", (label or "").lower())


def _find_column(columns: Sequence[str], keywords: Sequence[str]) -> str | None:
    """Locate the first column whose normalized key contains all keywords."""

    normalized = {_normalize_key(col): col for col in columns}
...
def parse_lineup_pdf(pdf_path: str | Path, teams: Iterable[str] | None = None) -> pd.DataFrame:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for idx, page in enumerate(pdf.pages):
            ...
            score_col = _find_column(df.columns, ["score"]) or next(
                (c for c in df.columns if "score" in _normalize_key(c)), None
            )
            ma_col = _find_column(df.columns, ["ma"]) or next(
                (c for c in df.columns if "m/a" in c.lower()), None
            )
            or_col = _find_column(df.columns, ["or"]) or next(
                (c for c in df.columns if c.lower() == "or"), None
            )
            dr_col = _find_column(df.columns, ["dr"]) or next(
                (c for c in df.columns if c.lower() == "dr"), None
            )
            as_col = _find_column(df.columns, ["as"]) or next(
                (c for c in df.columns if c.lower() == "as"), None
            )
            to_col = _find_column(df.columns, ["to"]) or next(
                (c for c in df.columns if c.lower() == "to"), None
            )
            st_col = _find_column(df.columns, ["st"]) or next(
                (c for c in df.columns if c.lower() == "st"), None
            )
            min_col = _find_column(df.columns, ["min"]) or next(
                (c for c in df.columns if c.lower() == "min"), None
            )

            if score_col is None or ma_col is None:
                raise ValueError(
                    "Could not locate required columns in table. Found headers: "
                    f"{list(df.columns)}"
                )
